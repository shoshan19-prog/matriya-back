...
router.post('/:layer', ensureDbInitialized, requireAdmin, async (req, res) => {
  ...
  try {
    const values = pickWritable(layer, req.body || {});
    const sourceId = await resolveSource(req.body || {}, { transaction: t });
    // Provenance principle: fact-stating layers may not create orphan knowledge.
    if (layer.sourceRequired && sourceId == null) {
      throw new HttpError(400, `Provenance required: provide 'source_id' or inline 'source' for layer '${req.params.layer}'`);
    }
    values.source_id = sourceId;
    values.confidence = req.body?.confidence ?? null;
    values.is_hypothesis = Boolean(req.body?.is_hypothesis) || false;
    // Separation guard — external domain is not negotiable from the API.
    values.knowledge_domain = KNOWLEDGE_DOMAIN_EXTERNAL;
    values.version = 1;
    const created = await layer.model.create(values, { transaction: t });
    await IklRecordHistory.create(
      { record_type: layer.recordType, record_id: created.id, version: 1, change_kind: 'create', snapshot: created.toJSON(), changed_by: req.user.id },
      { transaction: t }
    );
    await t.commit();
    res.status(201).json(created);
